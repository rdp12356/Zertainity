...
import agentSkills from './agent_skills.json' assert { type: 'json' };

// Find the highest quality skills from the extracted 88k+ database
const writingSkills = agentSkills; // Inject ALL skills per user request
...
function buildPrompt(c: CareerMatch, p: StudentProfile): string {
    ...
    if (writingSkills.length > 0) {
        skillInjection = `\n\nEXPERT AGENT SKILLS APPLIED:\nTo ensure the highest quality human-like output, apply these verified agent skills:\n`;
        writingSkills.forEach((s: any, i: number) => {
            skillInjection += `- Skill ${i+1}: ${s.name} from ${s.author}/${s.repo}\n`;
        });
        skillInjection += `Apply marketing-psychology and advanced editorial copywriting best practices automatically. Do not sound artificial.`;
    }
...
Be specific, encouraging, and honest. Second person. No bullet points in first paragraph.${skillInjection}`;
}
...
serve(async (req: Request) => {
    ...
    try {
        ...
        const profile: StudentProfile = {
            ...
            marks: typeof body.marks === 'object' && body.marks !== null ? body.marks : {},
            interestScores: typeof body.interestScores === 'object' && body.interestScores !== null ? body.interestScores : {},
        };
