...
serve(async (req: Request) => {
    ...
    try {
        ...
        const { resultId, question, history = [] } = await req.json();
        if (!resultId || !question) throw new Error("Missing resultId or question");
        ...
        if (!result) throw new Error("Career result not found or unauthorized");
