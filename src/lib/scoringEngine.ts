...
export interface CareerMatch {
    ...
    career: string;
    domain: string;
    matchScore: number;          // 0-100 weighted compatibility
    confidenceScore: number;     // 0-100 how much real signal vs defaults
    trendScore: number;          // 0-100 relative rank among all careers (Phase 3)
    strengths: string[];
    weakAreas: string[];
...
// Phase 3: Confidence score — measures how much real signal was provided
// vs how many signals defaulted to 50. Higher = more personalised.
function computeConfidenceScore(
    ...
    for (const signal of Object.keys(weights)) {
        // Signal is real if it was explicitly provided (not just defaulted)
        if (signals[signal] !== undefined) realSignals++;
    }
    return Math.round((realSignals / total) * 100);
}
...
export function scoreCareers(
    ...
    for (const [careerName, meta] of Object.entries(careerData)) {
        ...
        results.push({
            ...
            confidenceScore: computeConfidenceScore(signals, meta.weights),
            trendScore: 0, // computed after sort
            strengths,
            weakAreas,
    ...
    // Phase 3: trendScore — normalised position in the full ranked list (100 = top, 0 = bottom)
    const maxScore = results[0]?.rawScore ?? 100;
    const minScore = results[results.length - 1]?.rawScore ?? 0;
    ...
    const topCareers = results.slice(0, topN).map((r, i) => ({
        ...
        trendScore: Math.round(((r.rawScore - minScore) / range) * 100),
