...
interface CareerHistory {
  ...
  all_recommendations: Record<string, unknown>;
  created_at: string;
}
...
const Settings = () => {
  ...
  const loadProfile = async (userId: string) => {
    try {
      ...
      if (data) {
        setProfile({
          display_name: (data as Record<string, string>).display_name || "",
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .upsert({
          id: user.id,
          display_name: (profile as Record<string, string>).display_name || null,
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={(profile as Record<string, string>).display_name}
