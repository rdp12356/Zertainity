  ...
  return (
    <div className="min-h-screen flex bg-background/50 backdrop-blur-3xl">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center p-12 overflow-hidden border-r border-border/40">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/5 rounded-full mix-blend-screen filter blur-[100px] opacity-30" />
        <div className="absolute -bottom-32 left-20 w-[500px] h-[500px] bg-primary/10 rounded-full mix-blend-screen filter blur-[100px] opacity-30" />
        ...
        <div className="max-w-md w-full relative z-10 space-y-10">
          ...
          >
            <Link to="/" className="inline-flex items-center gap-2">
              <span className="text-sm font-semibold tracking-[0.15em] uppercase" style={{ color: 'var(--z-ink)' }}>
                Zertainity
              </span>
            </Link>
          </motion.div>
          ...
          >
            {features.map(({ icon: Icon, label }) => (
              <motion.div 
                ...
                className="group flex items-center gap-4 p-4 rounded-2xl bg-card/40 hover:bg-card/60 border border-border/40 hover:border-primary/30 backdrop-blur-md shadow-sm hover:shadow-md transition-all duration-300 cursor-default"
              >
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 group-hover:from-primary/30 group-hover:to-primary/10 flex items-center justify-center flex-shrink-0 transition-colors shadow-inner">
                  <Icon className="w-5 h-5 text-primary drop-shadow-sm" />
                </div>
                <span className="text-[15px] font-medium text-foreground/90 group-hover:text-foreground transition-colors">{label}</span>
              </motion.div>
            ))}
          </motion.div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
          ...
          {view !== "forgot" && (
            <div className="flex bg-muted/30 backdrop-blur-md rounded-xl p-1.5 border border-border/50 shadow-inner">
                  ...
                  <div className="space-y-4">
                    <form onSubmit={handleForgotPassword} className="space-y-4">
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 rounded-lg text-sm font-medium transition-all duration-200 active:scale-[0.98] disabled:opacity-60"
                      >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button
                        ...
                        disabled={loading}
                        className="h-12 rounded-xl border border-border/50 bg-background/50 hover:bg-muted/30 backdrop-blur-sm text-sm font-medium flex items-center justify-center gap-3 transition-all duration-300 hover:shadow-md hover:border-primary/30 group active:scale-[0.98] disabled:opacity-60"
                      >
                    ...
                    <form onSubmit={handleEmailAuth} className="space-y-4" id="auth-email-form">
                      ...
                      {view === "signup" && (
                        <div className="space-y-2">
                          <Label htmlFor="full-name" className="text-[13px] font-semibold tracking-wide uppercase text-muted-foreground">Full name</Label>
                          <Input
                            id="full-name"
                            type="text"
                            ...
                            disabled={loading}
                            autoComplete="name"
                            className="h-12 bg-muted/20 border-border/50 focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all shadow-sm rounded-xl"
                          />
                        </div>
                      )}
                      ...
                      {/* Email */}
                      <div className="space-y-2">
                        <Label htmlFor="auth-email" className="text-[13px] font-semibold tracking-wide uppercase text-muted-foreground">Email address</Label>
                        <Input
                          id="auth-email"
                          type="email"
                          placeholder="name@example.com"
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <Label htmlFor="auth-password" className="text-[13px] font-semibold tracking-wide uppercase text-muted-foreground">Password</Label>
                          {view === "login" && (
                            <button
                              type="button"
                        ...
                        <div className="relative">
                          <Input
                            ...
                            className="h-12 pr-12 bg-muted/20 border-border/50 focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all shadow-sm rounded-xl text-lg tracking-widest placeholder:tracking-normal placeholder:text-sm"
                          />
                          <button
                            type="button"
                            id="auth-toggle-password"
                            onClick={() => setShowPassword(!showPassword)}
                            className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-all"
                            tabIndex={-1}
                          >
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90 h-11 rounded-lg text-sm font-medium transition-all duration-200 active:scale-[0.98] disabled:opacity-60 shadow-sm mt-2"
                      >
                        {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                        {view === "login"
                          ? "Sign In"
