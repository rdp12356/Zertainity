            ...
            <div className="flex items-center gap-2 bg-muted/40 p-1.5 px-3 rounded-full border border-border/60">
              ...
              <Select value={activeOwnerEmail} onValueChange={(v) => { setActiveOwnerEmail(v); toast({ title: "Switched Owner", description: v }); }}>
                ...
                <SelectContent className="rounded-2xl">
                  {OWNER_EMAILS.map(e => <SelectItem key={e} value={e} className="text-xs font-mono">👑 {e}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
                ...
                <TableBody>
                  {filteredUsers.length === 0 ? (
                    ...
                    <TableRow key={u.id} className="border-b border-border/20">
                      <TableCell className="text-xs font-semibold">{u.display_name || <span className="text-muted-foreground italic">No name</span>} {isOwnerEmail(u.email) && <span className="text-amber-500 font-bold ml-1">👑</span>}</TableCell>
